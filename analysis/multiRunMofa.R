...
seurat_path = "data/ROI_seurat.rds"
output_dir = "data/results/multiProNor4000refactor4"
    top_n = 4000
    n_factors = 4
  ...
  protein_data <- GetAssayData(seurat_obj, assay = "Protein", slot = "data")
  ...
  protein_vst_mat <- as.matrix(protein_data)   # CLR-normalized ADT matrix
  ...
  dt <- as.data.frame(vst_mat) %>%
    ...
    mutate(view = "RNA_counts", group = .data[[MOFA_group]]) %>%
    select(sample, group, feature, view, value)
  rm(vst_mat); gc()  #


  dt_Protein <- as.data.frame(protein_vst_mat) %>%
    tibble::rownames_to_column("feature") %>%
    pivot_longer(-feature, names_to = "sample", values_to = "value") %>%
    left_join(meta, by = "sample") %>%
    mutate(view = "Protein",
           group = .data[[MOFA_group]]) %>%
    select(sample, group, feature, view, value)



  #dt_all 연결 실패
  dt_all <- bind_rows(dt, dt_Protein)

  #------------------------------------------------------------------------#
  # --- Create and train MOFA ---
  message("Preparing MOFA model...")
  mofa_obj <- create_mofa_from_df(dt_all)
  print(mofa_obj)
  ...
  view_name <- "RNA_counts"
  RNA_weights_df <- get_weights(mofa_obj, view = view_name, as.data.frame = TRUE)

  view_name <- "Protein"
  protein_weights_df <- get_weights(mofa_obj, view = "Protein", as.data.frame = TRUE)

  #------------------ save factor weight lst and factor weight figure ------------#
  for (factor_name in unique(RNA_weights_df$factor)) {
    sub_df <- filter(RNA_weights_df, factor == factor_name)
    write.csv(sub_df,
              file = file.path(factor_weight_dir, paste0("RNA_weights_", factor_name, ".csv")),
              row.names = FALSE)
  }

  # --- Factor weights plot ---
  for (i in seq_len(n_factors)) {
    p <- plot_top_weights(mofa_obj, view = "RNA_counts" ,factors = i, nfeatures = 20)
    ggsave(file.path(figure_dir, paste0("factor_weights_Factor", i, ".jpg")), p, dpi = 300)
  }

  for (factor_name in unique(protein_weights_df$factor)) {
    sub_df <- filter(protein_weights_df, factor == factor_name)
    write.csv(sub_df,
              file = file.path(factor_weight_dir, paste0("protein_weights_", factor_name, ".csv")),
              row.names = FALSE)
  }

  # --- Factor weights plot ---
  for (i in seq_len(n_factors)) {
    p <- plot_top_weights(mofa_obj, view = "Protein" ,factors = i, nfeatures = 20)
    ggsave(file.path(figure_dir, paste0("protein_factor_weights_Factor", i, ".jpg")), p, dpi = 300)
  }
